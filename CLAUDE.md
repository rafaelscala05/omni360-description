...
**Agente de Conteúdo conversacional**: o mesmo registry acima (`server/agent/registry.ts`) também alimenta um segundo consumidor — `provider: 'content'`, ferramentas em `server/agent/tools/{content,contentSeo}.ts`, cascas finas sobre `contentAgent.ts`/`seoAgent.ts`. Orquestrado por um **grafo LangGraph.js** (`server/agent/contentGraph.ts`), rodando como serviço próprio (`Dockerfile.contentAgent`) — não embutido no processo Express principal, já que a integração do CopilotKit fala com o LangGraph via HTTP, não em processo. O mesmo invariante de aprovação existe, mas via `interrupt()`/`Command(resume)` do LangGraph em vez de `agent_actions` — persistido por um checkpointer próprio no Firestore (`server/agent/firestoreCheckpointer.ts`, não existe um oficial). Modo automático/sempre-perguntar por ferramenta em `users/{uid}/agent_settings` (`server/agent/agentSettings.ts`); publicar, despublicar e conectar credencial são travas fixas, ignoram o modo automático. Ponte com o app principal em `server/copilotRuntime.ts` (`/api/copilotkit`, montada antes do `express.json()` global); frontend em `src/modules/content/chat/*` (`@copilotkit/react-core/v2`). Ver `CONTENT_MODULE.md` para o detalhe completo. O Agente Operacional **não foi migrado** para este motor — continua no loop de function-calling próprio acima; as duas superfícies de chat coexistem sem unificação por enquanto.
