...
- Tiny ERP (v2 token + v3 OAuth, selected per user; the UI currently only exposes v2) — `server/tinyAgent.ts` (v3 + shared types), `server/tinyV2.ts` (v2 client), `server/tinyProvider.ts` (version dispatch + `POST /api/tiny/push`); client: `src/services/tinyService.ts`, `src/components/integrations/TinyConnector.tsx`. Products tagged `_tinyProductId`.
  ...
  1. **The push only ever writes título, descrição complementar, SEO and imagens.** Fiscal/logistics fields (`ncm`, `gtin`, `cest`, `peso_liquido`, `peso_bruto`, dimensões) are echoed back from Tiny and never overridden — the ERP owns them, the local values come from spreadsheets/AI enrichment, and writing `ncm` makes Tiny re-derive `cest`, so a description push was silently rewriting tax data. `TinyPushProduct` deliberately has no fiscal fields, so this can't regress by accident from the client.
  2. **v2 `produto.alterar.php` reports the real reason in `registros[].registro.erros`, not at the top level.** It returns HTTP 200 with `retorno.status = "Erro"`, an empty top-level `erros` and no `codigo_erro`; `tinyV2CallRaw` must collect the per-record errors *before* deciding what to throw, or every failure surfaces as the useless `Tiny v2 status Erro (cod ?)`. `nome` is capped at 120 chars — over that, the title is skipped with an explanatory step instead of failing the whole record.
