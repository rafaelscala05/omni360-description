...
- **Push log** (`server/pushLog.ts`) — every ERP push (Wake, Tiny, Bling, IdWorks) returns `enviado?: PushLogEntry[]` alongside `steps`: the fields actually written, with the content that reached the ERP. Entries are appended **at the point the write decision is made** (inside the same `if` that sets the body key), never reconstructed afterwards, so the log cannot drift from the payload — the verify scripts assert log-and-body agree. Values are truncated to 600 chars server-side with the real size in `bytes`, because a 50-product push would otherwise ship megabytes of description HTML to the browser. `IntegrationSendPanel.tsx` renders it as a per-product collapsed section; the panel no longer auto-closes on success, since that hid the log. Wake's `buildProductPutBody` keeps its old signature (the operational agent tool calls it too), so its log is built in the push route instead.
