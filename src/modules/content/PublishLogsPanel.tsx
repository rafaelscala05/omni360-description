...
import type { PublishLog } from '../../services/contentService';
import { fetchPublishLogs } from '../../services/contentService';
...
const Linha: React.FC<{ log: PublishLog }> = ({ log }) => {
...
const PublishLogsPanel: React.FC<Props> = ({ projectId, aberto, onFechar }) => {
  ...
  const carregar = useCallback(async () => {
    ...
    setErro(null);
    try {
      setLogs(await fetchPublishLogs(projectId));
    } catch (e) {
      setErro(e instanceof Error ? e.message : 'Não consegui carregar os logs.');
    } finally {
      setCarregando(false);
    }
  }, [projectId]);
  ...
  const visiveis = apenasErros ? logs.filter((l) => !l.ok) : logs;
...
export default PublishLogsPanel;
