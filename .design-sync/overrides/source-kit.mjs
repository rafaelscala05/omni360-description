// forked from design-sync lib/source-kit.mjs — omni360-description's
// marketing components are all `export default function Name(...)` /
// `export default Name;`; the upstream synth entry's `export * from p`
// never re-exports defaults, so window.<Global> ended up with zero
// components. This fork additionally emits a named `export { default as
// Name }` per file when a PascalCase default-exported declaration is found.
//
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    // `export * from p` never forwards a `default` export — re-derive the
    // real declared name per file (same recovery as deriveComponentsFromSrc
    // below) and add a named re-export so default-exported components land
    // on window.<Global> too.
    ...
    for (const p of comps) {
      ...
      const realName = defaultDecls?.map((d) => d.getName?.()).find((n) => n && n !== 'default');
      if (realName && /^[A-Z][A-Za-z0-9]*$/.test(realName)) {
        entryLines.push(`export { default as ${realName} } from ${JSON.stringify(p)};`);
      }
    }
